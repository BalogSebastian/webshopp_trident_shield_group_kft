...
export default function AdminTridentShieldGroupKftEncoddedDashboard() {
  ...
  useEffect(() => {
    ...
    if (savedTab && ['dashboard', 'products', 'orders'].includes(savedTab)) {
  ...
  const loadData = async () => {
    try {
      ...
      // Termékek és rendelések API tömbként adja vissza, beszerzések API {data: ...} formátumban
  ...
  const handleProductUpdate = async (productId: string, updateData: any) => {
    try {
      setError("");
      setSuccessMessage("");
      ...
      setError(error.message || 'Hiba a frissítés során');
  ...
  const handleProductDelete = async (productId: string) => {
    ...
    try {
      ...
      setError(error.message || 'Hiba a törlés során');
      setTimeout(() => setError(""), 3000);
    }
  };
  ...
  const handleAddProduct = async () => {
    try {
      ...
      setError(error.message || 'Hiba a termék hozzáadása során');
      setTimeout(() => setError(""), 3000);
    }
  };
  ...
  const handleAddPurchase = async () => {
    try {
      ...
      setError(error.message || 'Hiba a beszerzés hozzáadása során');
      setTimeout(() => setError(""), 3000);
    }
  };

  const handleUpdatePurchase = async (id: string, updateData: any) => {
    try {
      setError("");
      setSuccessMessage("");
      ...
      setError(error.message || 'Hiba a beszerzés módosítása során');
      setTimeout(() => setError(""), 3000);
    }
  };
  ...
  const handleDeletePurchase = async (id: string) => {
    ...
    try {
      ...
      setError(error.message || 'Hiba a beszerzés törlése során');
      setTimeout(() => setError(""), 3000);
    }
  };
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-7xl mx-auto px-8 pt-8">
        ...
        <div className="flex space-x-1 bg-gray-100 p-1 rounded-xl">
          {['dashboard', 'products', 'orders', 'purchases'].map((tab) => (
            <button
              key={tab}
            ...
            >
              {tab === 'dashboard' ? 'Áttekintés' : tab === 'products' ? 'Termékek' : tab === 'orders' ? 'Rendelések' : 'Beszerzés'}
            </button>
          ))}
        </div>
      </div>
